...
class TestPet:
    ...
    def test_add_pet_new(self):
        ...
        with allure.step("Проверка параметров питомца в ответе"):
            assert response_json['id'] == payload['id'], "id питомца не совпал с ожидаемым"
            assert response_json['name'] == payload['name'], "name питомца не совпало с ожидаемым"
            assert response_json['category'] == payload['category'], "category питомца не совпала с ожидаемой"
            assert response_json['photoUrls'] == payload['photoUrls'], "photoUrls не совпал с ожидаемым"
            assert response_json['tags'] == payload['tags'], "tags не совпал с ожидаемым"
            assert response_json['status'] == payload['status'], "status не совпал с ожидаемым"
    ...
    @allure.title("Обновление ифнормации о питомце")
    def test_update_pet_by_id(self, create_pet):
        ...
        with allure.step("Проверка статуса ответа и данных питомца"):
            ...
            assert response.json()["name"] == payload["name"], "name питомца не совпало с ожидаемым"
            assert response.json()["status"] == payload["status"], "status не совпал с ожидаемым"
    ...
    def test_get_pets_by_incorrect_status(self, incorrect_status, status_code):
        with allure.step(f"Запрос с невалидным статусом {incorrect_status}"):
